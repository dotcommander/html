...
const (
	...
	DiagnosticImageOutsideRoot ImageDiagnosticCode = "image-outside-source"
)
...
func classifyInlineImagePath(baseDir, dest string) (string, string, ImageDiagnosticCode, bool) {
	...
	// Stdin has no source directory, so otherwise eligible local references stay
	// external without a degradation diagnostic. There is no ownership root
	// against which containment could be evaluated.
	if baseDir == "" {
		return "", "", "", false
	}
	p, contained, err := containedImagePath(baseDir, clean)
	if err != nil || !contained {
		return "", "", DiagnosticImageOutsideRoot, false
	}
	return p, mime, "", true
}
