...
- **Robust input-type detection** — `render.Detect` (`detect.go`) classifies a bounded scan window (64 KiB / 256 lines) as binary, Markdown, or plain. **Binary input is refused** (a NUL byte, or >10% non-text bytes) so a piped image/executable never renders as garbage — no flag overrides this. **Markdown requires a high-confidence structural signal** (a fenced code block, a GFM table, or a setext heading); weak inline cues (a `# comment` line, `__dunder__`, backticks, `arr[i](x)` reading as a link) are intentionally *not* decisive, so scripts, diffs, JSON/YAML, and logs stay plain. Trade-off: a heading-and-prose-only Markdown doc piped in renders plain unless you pass `-m`.
